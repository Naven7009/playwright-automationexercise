...
dotenv.config({ path: "credentials.env" }); // Load environment variables from .env
...
test.describe("Signup Page Testing", () => {
  ...
  test(
    ...
    async ({ signupPage }) => {
      ...
      await signupPage.fillSignupForm("automationexercise", email as string);
      const emailElement = await signupPage.page.locator(
        signupPage.signupEmail
      );

      if (await emailElement.isVisible()) {
        console.log(
          `Existing user detected: ${await emailElement.textContent()}`
        );
      } else {
        ...
        const accountCreatedMessage = await signupPage.page.locator(
